...
```powershell
./scripts/integration-ue58.ps1 -UProject ./MyGame/MyGame.uproject -Engine ./UE_5.8 -Clangd ./LLVM/bin/clangd.exe -Symbol SomeType
```

`-Engine` and `-Clangd` are optional verification inputs. Engine selection itself comes from the project's Unreal association; `doctor` selects a compatible clangd.
